...
export default function RegistrarPsicologo() {
  ...
  const [registrar, { loading, error }] = useMutation(REGISTRAR_PSICOLOGO, {
    onCompleted: (data) => {
      setSuccess(`Psicólogo "${data.registrarPsicologo.usuario.nombre}" registrado exitosamente.`);
  ...
  return (
    <Layout>
      <PageHeader
        ...
        subtitle="Agrega un nuevo profesional de salud mental al sistema"
      />
      ...
      <Card className={styles.card}>
        ...
        <form onSubmit={handleSubmit} className={styles.grid}>
          <Field label="Nombre completo">
            <input placeholder="Dr. Carlos Ruiz" value={form.nombre} onChange={set('nombre')} required />
          </Field>
          <Field label="Correo electrónico">
            <input type="email" placeholder="carlos@uni.edu" value={form.correo} onChange={set('correo')} required />
          </Field>
          ...
          <Field label="Cédula profesional">
            <input placeholder="12345678" value={form.cedula} onChange={set('cedula')} />
          </Field>
          <Field label="Especialidad">
            <input placeholder="Ansiedad y depresión" value={form.especialidad} onChange={set('especialidad')} />
          </Field>
          <Field label="Teléfono">
            <input placeholder="5559876543" value={form.telefono} onChange={set('telefono')} />
          </Field>
