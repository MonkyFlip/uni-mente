...
export const REGISTRAR_PSICOLOGO = gql`
  mutation RegistrarPsicologo($input: CreatePsicologoInput!) {
    registrarPsicologo(input: $input) {
      id_psicologo usuario { nombre correo } especialidad cedula telefono
    }
  }
`;
...
export const ACTUALIZAR_PSICOLOGO = gql`
  mutation ActualizarPsicologo($id: Int!, $input: UpdatePsicologoInput!) {
    actualizarPsicologo(id: $id, input: $input) {
      id_psicologo especialidad cedula telefono usuario { nombre correo }
    }
  }
`;
...
export const CREAR_HORARIO = gql`
  mutation CrearHorario($input: CreateHorarioInput!) {
    crearHorario(input: $input) { id_horario dia_semana hora_inicio hora_fin disponible }
  }
`;

export const ELIMINAR_HORARIO = gql`
  mutation EliminarHorario($id: Int!) { eliminarHorario(id: $id) }
`;
...
export const GET_MFA_ESTADO = gql`
  query MiEstadoMfa { miEstadoMfa { mfa_enabled } }
`;

export const SETUP_MFA = gql`
  mutation SetupMfa { setupMfa { qr_code secret } }
`;

export const HABILITAR_MFA = gql`
  mutation HabilitarMfa($input: VerificarMfaInput!) { habilitarMfa(input: $input) }
`;

export const DESHABILITAR_MFA = gql`
  mutation DeshabilitarMfa($input: VerificarMfaInput!) { deshabilitarMfa(input: $input) }
`;

export const CAMBIAR_PASSWORD = gql`
  mutation CambiarPassword($input: CambiarPasswordInput!) { cambiarPassword(input: $input) }
`;
...
export const RESTAURAR_BACKUP = gql`
  mutation RestaurarBackup($input: RestaurarBackupInput!) { restaurarBackup(input: $input) }
`;
